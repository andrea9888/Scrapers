...
def get_content(content_name, link1, a):
    ...
    try:
        ...
        return None
    if content_name == "Više detalja na":
        if "href" in html_list[last_change + 1]:
            position=html_list[last_change + 1].split("href=")[1].index("onclick")
            return html_list[last_change + 1].split("href=")[1][1:position-2]
...
while 1:
    try:
        ...
        for elem in link:
            ...
            current_ad = {}
            url=elem.a['href']
            source1=requests.get("https://www.realitica.com/hr/listing/1979846").text
            ...
            if help == 2:
                continue
            print("------0")
            #print(get_content("Više detalja na",link1, a))
            print("--------")
            ...
            for elem in list_of_fields:
                ...
                current_ad.update(a)
            list_of_optional = ["vrsta", "pordučje", "spavaćih soba", "kupatila", "cijena", "stambena površina", "parking mjesta", "zemljište", "od mora (m)", "klima uređaj", "novogradnja", "više detalja na"]
...
def get_web():
    list_of_sites = []
    for ad in mycol:
        if "više detalja na" in ad.keys():
            list_of_sites.append(ad)
    return ad
'''
